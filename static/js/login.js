var zyemail="",
    ...
    hash={
        ...
        'outlook.com': 'http://outlook.live.com',
...
function login_form_submit(){
    var $jsLoginBtn = $('#jsLoginBtn'),
        ...
        [
            {id: '#account_l', tips: Dml.Msg.epUserName, require: true},
    ...
    $.ajax({
        ...
        success: function(data) {
            if(data.username){
                Dml.fun.showValidateError($accountl, data.username);
            }else if(data.password){
                Dml.fun.showValidateError($passwordl, data.password);
            }else {
                if (data.status == "success") {
                    ...
                    if(data.msg == "用户未激活"){
                        ...
                        Dml.fun.showErrorTips($jsLoginTips, data.msg);
                    }
                }
            }
        },
...
function register_form_submit(){
    var $jsEmailRegBtn = $('#jsEmailRegBtn'),
        ...
        $accountl = $("#id_email"),
        $passwordl = $("#id_password"),
        $captchal = $("#id_captcha_1"),
        verify = verifyDialogSubmit(
        [
            {id: '#id_email', tips: Dml.Msg.epMail, errorTips: Dml.Msg.erMail, regName: 'email', require: true},
            {id: '#id_password', tips: Dml.Msg.epPwd, errorTips: Dml.Msg.erPwd, regName: 'pwd', require: true},
    ...
    $.ajax({
        ...
        dataType: 'json',
        url: "/register/",
        data: $('#jsEmailRegForm').serialize(),/* + '&' + '&autologin='+autoLogin + arg[0] + '=' + arg[1]*/
        async: true,
        beforeSend:function(XMLHttpRequest){
        ...
        success: function(data) {
            if(data.email){
                Dml.fun.showValidateError($accountl, data.email);
            }else if(data.password){
                Dml.fun.showValidateError($passwordl, data.password);
            }else if(data.captcha){
                Dml.fun.showValidateError($captchal, data.captcha);
            }else {
                if (data.status == "success") {
                    //注册账户处于未激活状态
                    zyemail = $accountl.val();
                    ...
                    Dml.fun.showDialog('#jsUnactiveForm');
                    Dml.fun.winReload(data.url, 5000);
                }
                else if(data.status == "fail"){
                    Dml.fun.showErrorTips($jsEmailTips, data.msg);
                }
            }
        },
        complete: function(XMLHttpRequest){
            $jsEmailRegBtn.val("注册");
...
$(function() {
    ...
    //注册刷新验证码点击事件
    $('#jsEmailRegForm .captcha-refresh').click({'form_id':'jsEmailRegForm'},refresh_captcha);
    $('#jsEmailRegForm .captcha').click({'form_id':'jsEmailRegForm'},refresh_captcha);
    ...
    //注册表单键盘事件
    $("#jsEmailRegForm").keydown(function(event){
        if(event.keyCode == 13){
